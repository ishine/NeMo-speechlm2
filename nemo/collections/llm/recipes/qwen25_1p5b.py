...
def model() -> run.Config[pl.LightningModule]:
    """
    Factory function to create a Qwen2.5 1.5b model configuration.

    Returns:
        run.Config[pl.LightningModule]: Configuration for the Qwen2.5 1.5b model.

    Examples:
        CLI usage:
            $ nemo llm pretrain model=qwen25_1p5b ...
...
def pretrain_recipe(
    ...
    """
    Create a pre-training recipe for Qwen2.5 1.5b model.
    ...
    Examples:
        CLI usage:
            $ nemo llm pretrain --factory qwen25_1p5b
            $ nemo llm pretrain --factory "qwen25_1p5b(num_nodes=1, name='my_qwen25_pretrain')"

        Python API usage:
            >>> recipe = pretrain_recipe(name="qwen25_pretrain", num_nodes=1)
            >>> print(recipe)
...
def finetune_recipe(
    ...
    """
    Create a fine-tuning recipe for Qwen2.5 1.5b model.
    ...
    Examples:
        CLI usage:
            $ nemo llm finetune --factory qwen25_1p5b

        Python API usage:
            >>> recipe = finetune_recipe(name="qwen25_1p5b_finetune", num_nodes=2)
            >>> print(recipe)
    ...
    recipe = default_finetune_recipe(
        model(), "Qwen/Qwen2.5-1.5B", dir, name, num_nodes, num_gpus_per_node, packed_sequence
    )
