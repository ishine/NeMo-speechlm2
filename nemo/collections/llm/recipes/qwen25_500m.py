...
def model() -> run.Config[pl.LightningModule]:
    """
    Factory function to create a Qwen2.5 500m model configuration.

    Returns:
        run.Config[pl.LightningModule]: Configuration for the Qwen2.5 500m model.

    Examples:
        CLI usage:
            $ nemo llm pretrain model=qwen25_500m ...
...
def pretrain_recipe(
    ...
    """
    Create a pre-training recipe for Qwen2.5 500m model.
    ...
    Examples:
        CLI usage:
            $ nemo llm pretrain --factory qwen25_500m
            $ nemo llm pretrain --factory "qwen25_500m(num_nodes=1, name='my_qwen25_pretrain')"
...
def finetune_recipe(
    ...
    """
    Create a fine-tuning recipe for Qwen2.5 500m model.
    ...
    Examples:
        CLI usage:
            $ nemo llm finetune --factory qwen2_500m

        Python API usage:
            >>> recipe = finetune_recipe(name="qwen25_500m_finetune", num_nodes=2)
            >>> print(recipe)
    ...
    recipe = default_finetune_recipe(
        model(), "Qwen/Qwen2.5-0.5B", dir, name, num_nodes, num_gpus_per_node, packed_sequence
    )
