...
def model() -> run.Config[pl.LightningModule]:
    """
    Factory function to create a Qwen3 8B model configuration.

    Returns:
        run.Config[pl.LightningModule]: Configuration for the Qwen3 8B model.

    Examples:
        CLI usage:
            $ nemo llm pretrain model=qwen3_8b ...
...
def pretrain_recipe(
    ...
    name: str = "default",
    # Trainer
    tensor_parallelism: int = 4,  # Default increased for 8B model
    ...
    """
    Create a pre-training recipe for Qwen3 8B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm pretrain --factory qwen3_8b
            $ nemo llm pretrain --factory "qwen3_8b(num_nodes=1, name='my_qwen3_pretrain')"
    ...
    return run.Partial(
        ...
        data=run.Config(
            ...
            tokenizer=run.Config(AutoTokenizer, "Qwen/Qwen3-8B"),
        ),
...
def finetune_recipe(
    ...
    """
    Create a fine-tuning recipe for Qwen3 8B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm finetune --factory qwen3_8b

        Python API usage:
            >>> recipe = finetune_recipe(name="qwen3_8b_finetune", num_nodes=2)
            >>> print(recipe)
    ...
    recipe = default_finetune_recipe(
        model(), "Qwen/Qwen3-8B", dir, name, num_nodes, num_gpus_per_node, packed_sequence
    )
