...
def model() -> run.Config[pl.LightningModule]:
    """
    Factory function to create a Qwen3 1.7B model configuration.

    Returns:
        run.Config[pl.LightningModule]: Configuration for the Qwen3 1.7B model.

    Examples:
        CLI usage:
            $ nemo llm pretrain model=qwen3_1p7b ...
...
def pretrain_recipe(
    ...
    """
    Create a pre-training recipe for Qwen3 1.7B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm pretrain --factory qwen3_1p7b
            $ nemo llm pretrain --factory "qwen3_1p7b(num_nodes=1, name='my_qwen3_pretrain')"
    ...
    return run.Partial(
        ...
        data=run.Config(
            ...
            tokenizer=run.Config(AutoTokenizer, "Qwen/Qwen3-1.7B"),
        ),
...
def finetune_recipe(
    ...
    """
    Create a fine-tuning recipe for Qwen3 1.7B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm finetune --factory qwen3_1p7b

        Python API usage:
            >>> recipe = finetune_recipe(name="qwen3_1p7b_finetune", num_nodes=2)
            >>> print(recipe)
    ...
    recipe = default_finetune_recipe(
        model(), "Qwen/Qwen3-1.7B", dir, name, num_nodes, num_gpus_per_node, packed_sequence
    )
