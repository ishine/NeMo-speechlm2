...
def model() -> run.Config[pl.LightningModule]:
    ...
    Examples:
        ...
        Python API usage:
            ...
            >>> print(model_config)
    """
    return run.Config(ReRankerModel, config=run.Config(Llama32Reranker1BConfig))
...
def finetune_recipe(
    ...
    # When using the Specter dataset, we need to set the num_hard_negatives to 1
    recipe.model.config.num_hard_negatives = 1
    recipe.data = run.Config(
        llm.SpecterReRankerDataModule,
