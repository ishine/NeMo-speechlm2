...
def model() -> run.Config[pl.LightningModule]:
    """
    Factory function to create a Gemma3 VL 12B model configuration.

    Returns:
        run.Config[pl.LightningModule]: Configuration for the Gemma3 VL 12B model.

    Examples:
        CLI usage:
            $ nemo llm pretrain model=gemma3vl_12b ...
    ...
    return run.Config(
        vlm.Gemma3VLModel,
        config=run.Config(
            vlm.Gemma3VLConfig12B,
            language_transformer_config=run.Config(llm.Gemma3Config12B),
...
def trainer(
    ...
    """
    Configure the NeMo Lightning Trainer for Gemma3 VL 12B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm pretrain trainer=gemma3vl_12b ...
...
def pretrain_recipe(
    ...
    """
    Create a pre-training recipe for Gemma3 VL 12B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm pretrain --factory gemma3vl_12b ...
            $ nemo llm pretrain --factory "gemma3vl_12b(num_nodes=2, name='my_pretrain')"

        Python API usage:
            >>> recipe = pretrain_recipe(name="gemma3vl_12b_pretrain", num_nodes=2)
            >>> print(recipe)
    """
    recipe = run.Partial(
        fn,
...
def pretrain_performance_optimizations(recipe: run.Partial) -> run.Partial:
    """
    Create a performance-optimized pre-training recipe for Gemma3 VL 12B model.
...
def finetune_recipe(
    dir: Optional[str] = None,
    name: str = "default",
    resume_path: Optional[str] = 'google/gemma-3-12b-it',
    num_nodes: int = 8,
    num_gpus_per_node: int = 8,
    peft_scheme: Optional[str] = 'none',
) -> run.Partial:
    """
    Create a fine-tuning recipe for Gemma3 VL 12B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm finetune --factory gemma3vl_12b

        Python API usage:
            >>> recipe = finetune_recipe(name="gemma3vl_12b_finetune", num_nodes=1)
            >>> print(recipe)
    """
    ...
    recipe = run.Partial(
        ...
        data=run.Config(
            ...
            micro_batch_size=1,
            tokenizer=run.Config(AutoTokenizer, pretrained_model_name='google/gemma-3-12b-it'),
            num_workers=4,
        ),
