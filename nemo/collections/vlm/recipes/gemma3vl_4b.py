...
def model() -> run.Config[pl.LightningModule]:
    ...
    return run.Config(
        vlm.Gemma3VLModel,
        config=run.Config(
            vlm.Gemma3VLConfig4B,
            language_transformer_config=run.Config(llm.Gemma3Config4B),
...
def finetune_recipe(
    dir: Optional[str] = None,
    name: str = "default",
    resume_path: Optional[str] = 'google/gemma-3-4b-it',
    num_nodes: int = 8,
    num_gpus_per_node: int = 8,
    ...
    recipe = run.Partial(
        ...
        data=run.Config(
            ...
            micro_batch_size=1,
            tokenizer=run.Config(AutoTokenizer, pretrained_model_name='google/gemma-3-4b-it'),
            num_workers=4,
        ),
