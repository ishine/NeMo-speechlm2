...
def get_eer(fpr, tpr, thresholds):
    """
    Calculate Equal Error Rate (EER) from false positive rate and true positive rate.
    ...
    # Equal Error Rate is where FPR = FNR
...
def get_min_dcf(fpr, tpr, thresholds, p_target=0.01, c_miss=1, c_fa=1):
    """
    Calculate minimum Detection Cost Function (minDCF) for speaker verification.

    Args:
        fpr (array): False positive rate values
        tpr (array): True positive rate values
        thresholds (array): Threshold values
        p_target (float): Prior probability of target speaker (default: 0.01)
        c_miss (float): Cost of miss (false rejection) (default: 1)
        c_fa (float): Cost of false alarm (false acceptance) (default: 1)

    Returns:
        min_dcf (float): Minimum Detection Cost Function value
        min_dcf_threshold (float): Threshold at minDCF
    """
    ...
    min_dcf = dcf[min_index]
    min_dcf_threshold = thresholds[min_index]

    # Normalize by the cost of always rejecting or always accepting
    dcf_norm = min(c_miss * p_target, c_fa * (1 - p_target))
    min_dcf_normalized = min_dcf / dcf_norm
