...
class PromptFormatter(ABC):
    ...
    * ``mask`` (boolean mask tensor of the same lenth as ``input_ids`` that's set to True on OUTPUT_ROLE turns)
    ...
    def encode_dialog(self, turns: list[dict]) -> dict[str, torch.Tensor]:
        ...
        # Filter out None values that may be returned by tokenizer
        # This can happen with certain tokenizers like Qwen when special tokens are not properly handled
        filtered_tokens = []
        filtered_turn_token_counts = []
        none_count_per_turn = [0] * len(turn_token_counts)
        ...
        for token in turn_tokens:
            if token is not None:
                ...
                current_turn_processed += 1
            else:
                # Count None tokens per turn for adjustment
                none_count_per_turn[current_turn_idx] += 1
                # Log warning about None token and skip it
                import logging
                logging.warning(f"Encountered None token in turn_tokens, skipping. "
                              f"This may indicate tokenizer configuration issues.")
            
            # Check if we've finished processing current turn
            if current_turn_processed + none_count_per_turn[current_turn_idx] >= turn_token_counts[current_turn_idx]:
                # Adjust turn token count by removing None tokens
                adjusted_count = turn_token_counts[current_turn_idx] - none_count_per_turn[current_turn_idx]
                filtered_turn_token_counts.append(adjusted_count)
                current_turn_idx += 1
                current_turn_processed = 0
        
        if not filtered_tokens:
            raise ValueError("All tokens were None after filtering. Check tokenizer configuration.")

        # Use adjusted token counts
        turn_token_counts = filtered_turn_token_counts
        ans = {"input_ids": torch.tensor(filtered_tokens, dtype=torch.long)}
