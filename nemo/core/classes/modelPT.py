...
class ModelPT(LightningModule, Model):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        if trainer is not None:
            # Check for both Lightning 2.x and 1.x Trainer types
            valid_trainer = isinstance(trainer, Trainer)
            if not valid_trainer:
                # Additional check for cross-version compatibility
                trainer_type_name = type(trainer).__module__ + '.' + type(trainer).__name__
                if 'pytorch_lightning' in trainer_type_name or 'lightning.pytorch' in trainer_type_name:
                    if 'Trainer' in trainer_type_name:
                        valid_trainer = True

            if not valid_trainer:
                raise ValueError(
                    f"trainer constructor argument must be either None or a PyTorch Lightning Trainer. "
